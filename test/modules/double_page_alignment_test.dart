...
void main() {
  ...
  group('Double Page Pair Construction (Issue #167)', () {
    test(
      'singleFirst = true correctly isolates first page and pairs the rest',
      () {
        const total = 5;
        final pair0 = getDoublePagePairIndices(
          pageViewIndex: 0,
          totalPages: total,
          singleFirst: true,
        );
        ...
        final pair1 = getDoublePagePairIndices(
          pageViewIndex: 1,
          totalPages: total,
          singleFirst: true,
        );
        ...
        final pair2 = getDoublePagePairIndices(
          pageViewIndex: 2,
          totalPages: total,
          singleFirst: true,
        );
  ...
  group('Double Page Startup & Mode Transition Consistency', () {
    ...
    test(
      'Toggling singleFirst while reading preserves the active page spread',
      () {
        ...
        expect(
          pvAfter,
          3,
        ); // (currentActual + 1) ~/ 2 = 7 ~/ 2 = 3 (pages 5 and 6)
  ...
  group(
    'Chapter-Aware Double Page Spreads (ReaderPageIndexMath - Issue #1008)',
    () {
      ...
      test(
        'Odd-length chapter endpoint does NOT pair with transition page (Issue #1008 Point 3)',
        () {
          // Chapter 1: 3 pages (indices 0, 1, 2)
          // Transition: 1 page (index 3)
          // Chapter 2: 2 pages (indices 4, 5)
          final pages = [
            makePage(ch1, 0, 0),
            makePage(ch1, 1, 1),
            makePage(ch1, 2, 2),
            makeTransition(ch1, ch2, 3),
            makePage(ch2, 4, 0),
            makePage(ch2, 5, 1),
          ];
          ...
          // Chapter 1 first spread: pages 0 and 1
          expect(spreads[0], const DoublePageSpread(0, 1));
          // Chapter 1 last page: page 2 alone (NOT paired with transition!)
          expect(spreads[1], const DoublePageSpread(2));
          expect(spreads[1].isSingle, isTrue);
          // Transition page: index 3 stands alone
          expect(spreads[2], const DoublePageSpread(3));
          expect(spreads[2].isSingle, isTrue);
          // Chapter 2 spread: pages 4 and 5
          expect(spreads[3], const DoublePageSpread(4, 5));
        },
      );

      test('Chapter-aware spreads with singleFirst = true', () {
        // Chapter 1: 3 pages (0, 1, 2)
        // Transition: (3)
        // Chapter 2: 2 pages (4, 5)
        final pages = [
          makePage(ch1, 0, 0),
          makePage(ch1, 1, 1),
          makePage(ch1, 2, 2),
        ...
        // Chapter 1: p0 alone, then p1 and p2 paired
        // Transition: p3 alone
        // Chapter 2: p4 alone, then p5 alone
        expect(spreads.length, 5);
        expect(spreads[0], const DoublePageSpread(0));
        expect(spreads[1], const DoublePageSpread(1, 2));
        expect(spreads[2], const DoublePageSpread(3));
        expect(spreads[3], const DoublePageSpread(4));
        expect(spreads[4], const DoublePageSpread(5));
      });

      test(
        'Adjacent chapters without transition page never pair across chapter boundary',
        () {
          // Chapter 1: 1 page (0)
          // Chapter 2: 1 page (1)
          final pages = [makePage(ch1, 0, 0), makePage(ch2, 1, 0)];

          final spreads = ReaderPageIndexMath.buildSpreads(pages);
          expect(spreads.length, 2);
          expect(spreads[0], const DoublePageSpread(0));
      ...
      test(
        'ReaderPageIndexMath round-trip indexing with chapter transitions',
        () {
          final pages = [
            makePage(ch1, 0, 0),
            makePage(ch1, 1, 1),
          ...
          expect(math.pageViewPageCount, 4);

          // actualToPageViewIndex
          expect(math.actualToPageViewIndex(0), 0);
          expect(math.actualToPageViewIndex(1), 0);
          expect(math.actualToPageViewIndex(2), 1); // solo page 2
          expect(math.actualToPageViewIndex(3), 2); // transition page
          expect(math.actualToPageViewIndex(4), 3); // ch2 page 0
          expect(math.actualToPageViewIndex(5), 3); // ch2 page 1

          // pageViewToActualIndex
          expect(math.pageViewToActualIndex(0), 0);
          expect(math.pageViewToActualIndex(1), 2);
          expect(math.pageViewToActualIndex(2), 3);
          expect(math.pageViewToActualIndex(3), 4);

          // currentIndexLabel
          expect(math.currentIndexLabel(0, 3), '1-2');
          expect(math.currentIndexLabel(1, 3), '1-2');
          expect(math.currentIndexLabel(2, 3), '3');
          expect(
            math.currentIndexLabel(3, 3),
            '',
          ); // transition page label is empty
          expect(math.currentIndexLabel(4, 2), '1-2');
        },
      );

      test('Synthetic spreads fallback when pages is omitted', () {
        final math = ReaderPageIndexMath(
          isDoublePageActive: true,
          singleFirst: false,
          pageCount: 5,
        ...
        expect(math.pageViewPageCount, 3);
        expect(math.spreads, const [
          DoublePageSpread(0, 1),
          DoublePageSpread(2, 3),
          DoublePageSpread(4),
        ]);

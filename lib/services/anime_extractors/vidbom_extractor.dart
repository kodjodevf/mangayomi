...
class VidBomExtractor {
  ...
  Future<List<Video>> videosFromUrl(String url) async {
    try {
      ...
      final script = xpathSelector(response.body).queryXPath('//script[contains(text(), "sources")]/text()').attrs;

      final data = script.first!.substringAfter('sources: [').substringBefore('],');

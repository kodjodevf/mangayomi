...
class JsDomSelector {
  ...
  void init() {
    ...
    runtime.onMessage('ele_get_elements_by', (dynamic args) {
      ...
      final element = _elements[key];
      final elements = switch (type) {
        'children' => element?.children.where((e) => e.localName == name).toList(),
    ...
    runtime.evaluate('''
function cheerioLoad(content, options, isDocument) {
  return new CheerioAPI(content);
}
...
class CheerioAPI {
  ...
  select(selector) {
    ...
    return new CheerioDoc(elements);
  }
}
...
class CheerioDoc {
    ...
    children(selector) {
        return this.getElementsListBy('children', selector);
    }
